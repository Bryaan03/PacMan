...
public abstract class AbstractAnimated implements IAnimated {
     ...
     * @return true or false si y'a une collision avec un mur
     */
    public boolean canMoveTo(String direction, double distance) {
        // Vérifie les limites du jeu
        double testX = xPosition.get();
        double testY = yPosition.get();

        // On "preshot" la position future selon la direction
        switch (direction) {
            case "LEFT" -> testX -= distance;
            case "RIGHT" -> testX += distance;
            case "UP" -> testY -= distance;
            case "DOWN" -> testY += distance;
            default -> throw new IllegalArgumentException("Direction inconnue : " + direction);
        }

        // Si on sort des limites ou qu’il y a un mur → impossible
        return !isOnWall((int)testX, (int)testY);
    }
    ...
    private boolean isOnWall(int x, int y) {
        return ((game.getCellAt(x + MARGIN, y + MARGIN).getWall() != null) ||
                (game.getCellAt(x + MARGIN, y + getHeight() - MARGIN).getWall() != null) ||
                (game.getCellAt(x + getWidth() - MARGIN, y + MARGIN).getWall() != null) ||
                (game.getCellAt(x + getWidth() - MARGIN, y + getHeight() - MARGIN).getWall() != null));
    }
	 ...
	 */
	@Override
    public void onCollisionWith(PacGomme other) {
	 // Intentionally left blank
	}
	 ...
	 */
	@Override
    public void onCollisionWith(IBonus other) {
	 // Intentionally left blank
	}
	 ...
	 */
	@Override
    public void onCollisionWith(Wall other) {
	 // Intentionally left blank
	}
	
    /**
    * Gère la collision avec une méga-gomme.
    *
    * @param gum l'objet méga-gomme.
    */
	...
	/**
	 * Modifie l'état de cet objet lorsque celui-ci est entré en collision avec un
	 * autre objet. Lors de l'appel à cette méthode, il est garanti que les deux
	 * objets sont entrés en collision. Il n'est en particulier pas nécessaire
	 * d'utiliser {@link #isCollidingWith(IAnimated)} pour s'en assurer.
	 *
